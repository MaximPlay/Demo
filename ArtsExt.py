...
class ArtsMod(loader.Module):
    ...
    async def priletelcmd(self, message):
        ...
        text = utils.get_args_raw(message)
        if not text:
            text = ("ИГОРЬ ЛЮБИТ СОСАТЬ ХУИ, А ТЫ?!")
            prilitel = ("▬▬▬.◙.▬▬▬\n"
                        "  ═▂▄▄▓▄▄▂\n"
                        "◢◤ █▀▀████▄▄▄▄◢◤\n"
                        "█▄ █ █▄ ███▀▀▀▀▀▀▀╬\n"
    ...
    async def fckucmd(self, message):
        """Используй .fcku."""
        fcku = ("▂▂▂▂▂▂╭━╮▂▂▂▂▂▂▂\n"
                     "▂▂▂▂▂▂┣━┫▂▂▂▂▂▂▂ \n"
                     "▂▂▂▂▂▂┃╭┃▂▂▂▂▂▂▂ \n"
                     ...
                     "▂▂▂▏┃┈┃┈┃┈┃┈┃▂▂▂\n" 
                     "▂▂▂╰╮▂▂▂▂▂▂╭╯▂▂▂")
        await message.edit(fcku) 

    async def huy2cmd(self, message):
